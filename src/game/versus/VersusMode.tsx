...
export function VersusMode({ onExit }: { onExit: () => void }) {
  ...
  const isLobby = Boolean(session && room && (room.status === "waiting" || room.status === "ready") && !room.currentAugmentRound);
  ...
  const buildAttackBurst = (kind: "manual" | "bigFruit", level?: FruitLevel) => {
    if (!session) return [];
    const baseType: AttackType = kind === "manual" ? (["board-shake", "hide-next", "gravity-boost"][Math.floor(Math.random() * 3)] as AttackType) : level && level >= 12 ? "heavy-junk" : level && level >= 10 ? "gravity-boost" : "garbage-fruit";
    const events: VersusAttackEvent[] = [];
    if (kind === "manual") events.push(attackEvent(session.playerId, "garbage-fruit", getRandomGarbageFruit()));
